...
uses
  ...
  Buttons, cxLookupEdit, cxDBLookupEdit, cxDBLookupComboBox, AdvGlassButton,
...
type
  TParkirFrm = class(TForm)
    ...
    grdDTBV_MasterColumn1: TcxGridDBColumn;
    grdDTBV_MasterColumn2: TcxGridDBColumn;
    Label6: TLabel;
    Label7: TLabel;
    edtJnsKendaraan: TcxDBTextEdit;
    Label8: TLabel;
    Label9: TLabel;
    edtJnsAnggota: TcxDBTextEdit;
    CheckBox1: TcxCheckBox;
    MemKarcisHilangtype_kendaraan: TStringField;
    MemKarcisHilangjns_pengguna: TStringField;
    qAnggotatype_kendaraan: TStringField;
    qMember: TZQuery;
    qMemberno_kartu_char: TStringField;
    qMemberno_kartu_hex: TStringField;
    qMemberid_jns_anggota: TStringField;
    qMemberid_jns_kendaraan: TStringField;
    qMembernama_anggota: TStringField;
    qMemberalamat: TStringField;
    qMemberkota: TStringField;
    qMembernopol: TStringField;
    qMemberisactive: TStringField;
    qMemberusr_ins: TStringField;
    qMemberusr_upd: TStringField;
    qMemberdt_ins: TDateTimeField;
    qMemberdt_upd: TDateTimeField;
    qMemberdt_mulai: TDateTimeField;
    qMemberdt_akhir: TDateTimeField;
    qMembernomor: TIntegerField;
    qMemberstatus_aktif: TBooleanField;
    qMembernik: TStringField;
    qMembertype_kendaraan: TStringField;
    edtNamaMember: TcxDBTextEdit;
    Label10: TLabel;
    Label13: TLabel;
    MemKarcisHilangnama_member: TStringField;
    LblTarif: TLabel;
    qryTrfKarcisHilangtarif: TFloatField;
    qLastGateOutusr_upd: TStringField;
    qLastGateOuttype_kendaraan: TStringField;
    frxdbKarcisHilang: TfrxDBDataset;
    MemKarcisHilangno_karcis: TStringField;
    ...
    qPrinttype_kendaraan: TStringField;
    qPrintnama_member: TStringField;
    qKarcisHilangtype_kendaraan: TStringField;
    MemKarcisHilangdt_ins: TDateTimeField;
    btnKHSimpan: TAdvGlassButton;
    btnKHBatal: TAdvGlassButton;
    btnPendLain: TRzButton;
    RzLabel3: TRzLabel;
    qLastGateOutketerangan: TMemoField;
    RzPanel19: TRzPanel;
    RzPanel21: TRzPanel;
    btnKeluar: TAdvGlassButton;
    btnPenjualanVoucher: TAdvGlassButton;
    edtNoMember: TcxTextEdit;
    qKarcisHilangtarif: TFloatField;
    memPrint: TkbmMemTable;
    memPrintnopol: TStringField;
    memPrintno_ktp: TStringField;
    memPrintnama: TStringField;
    memPrintalamat: TStringField;
    memPrintno_hp: TStringField;
    memPrinttype_kendaraan: TStringField;
    memPrintjns_pengguna: TStringField;
    memPrintnama_member: TStringField;
    memPrintno_karcis: TStringField;
    memPrinttarif: TStringField;
    memPrinttgl: TStringField;
    memPrintjam: TStringField;
    grdDTBV_MasterColumn3: TcxGridDBColumn;
    qLastGateOutnopol: TStringField;
    qLastGateOutjam_masuk: TDateTimeField;
    qLastGateOutnama_anggota: TStringField;
    qLastGateOutno_kartu_char: TStringField;
    qLastGateOutid_jns_anggota: TStringField;
    CtrlDel: TLMDGlobalHotKey;
    F11: TLMDGlobalHotKey;
    frxKarcis0: TfrxReport;
    Timer3: TTimer;
    ...
    procedure F10KeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure CtrlDelKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure edtNoMemberKeyPress(Sender: TObject; var Key: Char);
    procedure btnKHSimpanClick(Sender: TObject);
    procedure edtJnsKendaraanKeyPress(Sender: TObject; var Key: Char);
  ...
  private
    ...
    vvoucher, vmrs, vnama, dirname_out, dir_folder_ot, vnoKarcis : String;
    vsisa : double;

    cNoKarcis, cNopol, cOprtr, cGateID, cCustm, cTypeKnd, cMemberName, cTarif, cHrTgl, cJam : String;
    cShift: Integer;
    ...
    procedure CetakStruk(vNo, vJns:String);
    ...
    procedure CaptureIPCam(vNo : String);
    procedure InsertData(vNoKarcis:String; vNumb:Integer);
    procedure Simpan(vGate:String);
...
procedure TParkirFrm.InsertData(vnoKarcis:String; vNumb:Integer);
var
s, vNopol, vOprtr, vGateID, vCustm, vTypeKnd, vMemberName, vNoKartu: String;
vShift: Integer;
vTarif : Double;
begin

  try
      if MemKarcisHilang.State=dsEdit then
         MemKarcisHilang.Post;


      vNopol:= MemKarcisHilangnopol.AsString;
      vOprtr:= DM.UserConnect;
      vGateID:= DM.gate_id;
      vCustm:= MemKarcisHilangjns_pengguna.AsString;
      vTypeKnd:= MemKarcisHilangtype_kendaraan.AsString;
      vMemberName:= MemKarcisHilangnama_member.AsString;
      vShift:= DM.GetShift;
      vTarif:= MemKarcisHilangtarif.AsFloat;
      vNoKartu:= MemKarcisHilangno_ktp.AsString;
      ...
      cCustm:= vCustm;
      cTypeKnd:= vTypeKnd;
      cMemberName:= vMemberName;
      cTarif:= FormatFloat(',0', vTarif);
      cHrTgl:= FormatDateTime('dddd, dd-mm-yyyy', Now);
      cJam:= FormatDateTime('hh:mm:ss', Now);
      cShift:= vShift;

      if vNumb=1 then begin
        s:= 'select trans.fn_proc_ins_kartu_hilang_petugas('+QuotedStr(vnoKarcis)+',';
        s:= s+QuotedStr('R4')+','+QuotedStr(vNopol)+',';
        s:= s+QuotedStr(DM.UserConnect)+','+QuotedStr(vGateID)+','+IntToStr(vShift)+',';
        s:= s+QuotedStr(vCustm)+','+QuotedStr(vTypeKnd)+','+QuotedStr(vMemberName)+','+QuotedStr(vNoKartu)+')';
        DM.PrepareQuery(qExec);
        DM.ExecQuery(qExec,s);
      end
      else
      if vNumb=2 then begin
        s:= 'select trans.fn_proc_ins_kartu_hilang_premium('+QuotedStr(vnoKarcis)+',';
        s:= s+QuotedStr('R4')+','+QuotedStr(vNopol)+',';
        s:= s+QuotedStr(DM.UserConnect)+','+QuotedStr(vGateID)+','+IntToStr(vShift)+',';
        s:= s+QuotedStr(vCustm)+','+QuotedStr(vTypeKnd)+')';
        DM.PrepareQuery(qExec);
        DM.ExecQuery(qExec,s);
        
        CetakStruk(vnoKarcis, 'trans');
      end;
...
procedure TParkirFrm.CaptureIPCam(vNo : String);
begin
  try
    DM.vSysDate.Close;
    DM.vSysDate.Open;
    ...
    IpCamKeluar.SnapShot(ChangeFileExt(Trim(dirname_out+vNo), '.jpeg'),476,278);
    LblGateKeluar.Caption:= Trim(dirname_out+vNo);
  except
    on E: Exception do begin
      DM.MyMsg(mmError,'Error has been encountered !',E.Message)
    end
...
procedure TParkirFrm.CetakStruk(vNo, vJns:String);
begin

  if vJns= 'trans' then begin
      try
        ...
        memPrint.EmptyTable;
        memPrint.Append;
        memPrintno_karcis.AsString:= vNo;
        memPrintnopol.AsString:= MemKarcisHilangnopol.AsString;
        memPrintno_ktp.AsString:= '-';
        memPrintnama.AsString:= DM.UserConnect;
        memPrintalamat.AsString:= '-';
        memPrintno_hp.AsString:= '-';
        memPrinttarif.AsString:= FormatFloat(',0', MemKarcisHilangtarif.AsFloat);
        memPrinttgl.AsString:= FormatDateTime('dddd, dd-mm-yyyy', Now);
        memPrintjam.AsString:= FormatDateTime('hh:mm:ss', Now);
        memPrinttype_kendaraan.AsString:= MemKarcisHilangtype_kendaraan.AsString;
        memPrintjns_pengguna.AsString:= MemKarcisHilangjns_pengguna.AsString;
        if MemKarcisHilangjns_pengguna.AsString= 'UMUM' then
           memPrintnama_member.AsString:= Trim('--CASH')
        else
           memPrintnama_member.AsString:= MemKarcisHilangnama_member.AsString;
  ...
  if vJns= 'ulang' then begin
      try
        ...
        memPrint.Append;
        memPrintno_karcis.AsString:= vNo;
        memPrintnopol.AsString:= qLastGateOutnopol.AsString;
        memPrintnama.AsString:= qLastGateOutusr_upd.AsString;
        memPrinttarif.AsString:= FormatFloat(',0', qLastGateOuttarif.AsFloat);
        memPrinttgl.AsString:= FormatDateTime('dddd, dd-mm-yyyy', qLastGateOutjam_masuk.AsDateTime);
        memPrintjam.AsString:= FormatDateTime('hh:mm:ss', qLastGateOutjam_masuk.AsDateTime);
        memPrinttype_kendaraan.AsString:= qLastGateOuttype_kendaraan.AsString;
        memPrintjns_pengguna.AsString:= qLastGateOutid_jns_anggota.AsString;
        if qLastGateOutid_jns_anggota.AsString= 'UMUM' then
           memPrintnama_member.AsString:= Trim('--CASH')
        else
           memPrintnama_member.AsString:= qLastGateOutnama_anggota.AsString;
...
//  ImageMasuk.Picture.LoadFromFile('Images\tidakadafoto.jpg');
  if DM.gate_id='61' then begin
     IpCamKeluar.Play('rtsp://'+_IPCamLive);
     cxLabel8.Caption:= 'KEBERANGKATAN';
  end
  else
  if DM.gate_id='62' then begin
//     IpCamKeluar.Play('rtsp://'+_IPCamLive);
     cxLabel8.Caption:= 'KEDATANGAN';
  end;
...
begin
    ...
    qBukaPalang.Append;
    qBukaPalangno_karcis.AsString:= vnoKarcis;
    qBukaPalangid_jns_anggota.AsString:= 'UMUM';
...
begin
   vnoKarcis:= Trim(qLastGateOutno_karcis.AsString);
   CetakStruk(vnoKarcis, 'ulang');
end;

procedure TParkirFrm.CtrlDelKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
vnoKarcis : String;
begin
//  if DM.gate_id='61' then begin
//
//    if DM.UsrPermission = '1' then begin
//
//
//      if (edtKHNopol.Text='') and (edtJnsKendaraan.Text='') then begin
//          DM.MyMsg(mmInformation,'No. Pol Kendaraan / Type Kendaraan Belum diisi..!!','');
//          Exit;
//      end;
//
//      try
//        vnoKarcis:= trim(DM.vNoKarcis);
//        MemKarcisHilangno_karcis.AsString:= vnoKarcis;
//        MemKarcisHilangdt_ins.AsDateTime:= Now;
//        MemKarcisHilangnama.AsString:= DM.UserConnect;
//        MemKarcisHilangtype_kendaraan.AsString:= edtJnsKendaraan.Text;
//        MemKarcisHilangnopol.AsString:= edtKHNopol.Text;
//        MemKarcisHilangtarif.AsFloat:= qryTrfKarcisHilangtarif.AsFloat;
//
//        CaptureIPCam(vnoKarcis);
//
//        qKarcisHilang.Close;
//        qKarcisHilang.Open;
//        qKarcisHilang.Append;
//        qKarcisHilangnopol.AsString:= MemKarcisHilangnopol.AsString; //no pol kendaraan
//        qKarcisHilangno_ktp.AsString:= MemKarcisHilangno_karcis.AsString; //no karcis
//        qKarcisHilangnama.AsString:= IntToStr(DM.GetShift); //shift
//        qKarcisHilangalamat.AsString:= Trim(vnoKarcis+'.jpeg'); //file capture
//        qKarcisHilangno_hp.AsString:= MemKarcisHilangjns_pengguna.AsString; //jenis pelanggan
//        qKarcisHilangtype_kendaraan.AsString:= MemKarcisHilangtype_kendaraan.AsString; //type kendaraan
//        qKarcisHilangtarif.AsFloat:= MemKarcisHilangtarif.AsFloat; //tarif parkir
//        qKarcisHilangusr_ins.AsString:= DM.UserConnect; //kasir/Operator
...
//        qKarcisHilangno_ktp.AsString:= MemKarcisHilangno_karcis.AsString;
//        qKarcisHilangnama.AsString:= IntToStr(DM.GetShift);
//        qKarcisHilangalamat.AsString:= Trim(vnoKarcis+'.jpeg');
//        qKarcisHilangno_hp.AsString:= MemKarcisHilangjns_pengguna.AsString;
//        qKarcisHilangtype_kendaraan.AsString:= MemKarcisHilangtype_kendaraan.AsString;
//        qKarcisHilangtarif.AsFloat:= MemKarcisHilangtarif.AsFloat;
...
begin
    try
      ...
      qryTrfKarcisHilang.Params.ParamByName('jns_anggota').Value:= 'UMUM';
      qryTrfKarcisHilang.Open;
      MemKarcisHilang.Close;
      MemKarcisHilang.Open;
      MemKarcisHilang.Append;
      MemKarcisHilangjns_pengguna.AsString:= 'UMUM';
      MemKarcisHilangtarif.AsFloat:= qryTrfKarcisHilangtarif.AsFloat;
      LblTarif.Caption:= FormatCurr('#,###',qryTrfKarcisHilangtarif.AsFloat);
    except
      on E: Exception do
        DM.MyMsg(mmError,'Error has been encountered !',E.Message);
    end;
//    edtNoMember.Visible:= False;
    edtNoMember.SetFocus;
end;
...
procedure TParkirFrm.CheckBox1PropertiesChange(Sender: TObject);
begin

    if CheckBox1.Checked=True then begin
     try
      qryTrfKarcisHilang.Close;
      qryTrfKarcisHilang.Params.ParamByName('jns_anggota').Value:= 'PETUGAS';
      qryTrfKarcisHilang.Open;
     except
      on E: Exception do begin
        DM.MyMsg(mmError,'Error has been encountered !',E.Message);
      ...
      MemKarcisHilang.Open;
      MemKarcisHilang.Append;
      MemKarcisHilangjns_pengguna.AsString:= 'MEMBER';
      MemKarcisHilangtarif.AsFloat:= qryTrfKarcisHilangtarif.AsFloat;
      LblTarif.Caption:= FormatCurr('#,###',qryTrfKarcisHilangtarif.AsFloat);

      MemKarcisHilangnama_member.AsString:= '';
      MemKarcisHilangtype_kendaraan.AsString:= '';
      MemKarcisHilangnopol.AsString:= '';
      MemKarcisHilangnama.AsString:= DM.UserConnect;

      edtNamaMember.Visible:= True;
      Label10.Visible:= True;
      Label13.Visible:= True;

      edtNamaMember.Enabled:= False;
      edtKHNopol.Enabled:= False;
      edtJnsKendaraan.Enabled:= False;
      edtJnsAnggota.Enabled:= False;
    ...
    if CheckBox1.Checked=False then begin
     try
      ...
      edtNamaMember.Visible:= False;

      edtNamaMember.Enabled:= True;
      edtKHNopol.Enabled:= True;
      edtJnsKendaraan.Enabled:= True;
      edtJnsAnggota.Enabled:= True;
...
procedure TParkirFrm.edtNoMemberKeyPress(Sender: TObject; var Key: Char);
begin

  if key=#13 then begin
     CheckBox1.Checked:= True;
     qCekAnggotaAktif.Close;
     qCekAnggotaAktif.Params.ParamByName('pkartu').Value:= edtNoMember.Text;
     qCekAnggotaAktif.Open;
     if qCekAnggotaAktifstatus.Value=1 then begin

         if CekAnggota(edtNoMember.Text) then begin
            qMember.Close;
            qMember.Params.ParamByName('nokartu').Value:= edtNoMember.Text;
            qMember.Open;

            try

              MemKarcisHilangnopol.AsString:= qMembernopol.AsString;
              MemKarcisHilangtype_kendaraan.AsString:= qMembertype_kendaraan.AsString;
              MemKarcisHilangnama_member.AsString:= qMembernama_anggota.AsString;
              MemKarcisHilangno_ktp.AsString:= edtNoMember.Text;
            ...
            DM.MyMsg(mmInformation,'MEMBER TIDAK DIKENALI..!!!','');
            edtNoMember.SetFocus;
            Exit;
         end;
      ...
      DM.MyMsg(mmInformation,'KARTU MEMBER SUDAH TIDAK AKTIF..!!!'#13'SEGERA HUBUNGI ADMINISTRATOR..!!','');
      Exit;
      edtNoMember.SetFocus;
      Sleep(1000);
      edtNoMember.Text:= '';
...
procedure TParkirFrm.Simpan(vGate:String);
VAR
vNumb : Integer;
begin
  ...
  try
    vnoKarcis:= trim(DM.vNoKarcis);
    if vGate='61' then begin
      CaptureIPCam(vnoKarcis);
      if CheckBox1.Checked=True then begin
        vNumb:= 1;
        InsertData(vnoKarcis, vNumb);
      end
      else
      if CheckBox1.Checked=False then begin
        vNumb:= 2;
...
procedure TParkirFrm.edtJnsKendaraanKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key=#13 then
     btnKHSimpan.SetFocus;
end;
...
procedure TParkirFrm.Timer3Timer(Sender: TObject);
begin
  if DM.gate_id='61' then begin
      if edtKHNopol.Text='' then
         CheckBox1.Checked:= True;
  end else begin
