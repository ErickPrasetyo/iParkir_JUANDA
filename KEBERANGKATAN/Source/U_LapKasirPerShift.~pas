...
uses
  ...
  cxDBLookupComboBox, cxRadioGroup, cxGridDBBandedTableView, dxSkinBlack,
...
type
  TLapKasirPerShiftFrm = class(TForm)
    ...
    qTimeShift: TZQuery;
    MemMasterListdt_from_shift: TStringField;
    MemMasterListdt_end_shift: TStringField;
    qTimeShiftshift: TIntegerField;
    qTimeShifttawal: TTimeField;
    qTimeShifttakhir: TTimeField;
    Detailketerangan: TStringField;
    Detailtype_kendaraan: TStringField;
    Detailnama_anggota: TStringField;
    qJmlPelanggan: TZQuery;
    qJmlPelangganjml_umum: TLargeintField;
    qJmlPelangganjml_member: TLargeintField;
    qJmlPelangganjml_all: TLargeintField;
    frxdbJumlah: TfrxDBDataset;
    Detailnm_anggota: TStringField;
    Detailno_kartu: TStringField;
    OpenDialog1: TSaveDialog;
    frxRekapKarcis0: TfrxReport;
...
begin
  ...
  LapKasirPerShiftFrm.btnExport.Visible:= False;
  LapKasirPerShiftFrm.edtPetugas.Text:= 'SEMUA';
end;
...
var s, ptgl, pshift, where_str, pgate : String;
begin
   ptgl:= QuotedStr(FormatDateTime('dd-mm-yyyy',dtpStart.Date)) ;
   pshift:= QuotedStr(edtShift.Text);
   pgate:= QuotedStr(DM.gate_id);
...
//      where_str:= 'AND a.usr_upd='+QuotedStr(Trim(edtPetugas.Text));
    ...
    s:= 'select a.*, g.keterangan from public.fn_get_total_shift('+ptgl+','+pshift+','+pgate+') a ';
...
//      where_str:= 'AND a.usr_upd='+QuotedStr(Trim(edtPetugas.Text));
    ...
    s:= 'select a.*,';
    s:= s+' (case when a.id_jns_anggota='+QuotedStr('UMUM')+' then '+QuotedStr('CASH')+' else a.nama_anggota end) as nm_anggota,';
    s:= s+' (case when a.id_jns_anggota='+QuotedStr('UMUM')+' then a.no_karcis else a.no_kartu_char end) as no_kartu';
    s:= s+' from  public.fn_get_detail_shift('+ptgl+','+pshift+','+pgate+') as a';
    s:= s+' WHERE 1=1 '+where_str;
...
var
Printer, dt, Sft, gt_id : string;
begin

   dt:= FormatDateTime('dd-mm-yyyy', dtpStart.Date);
   Sft:= Trim(edtShift.Text);
   gt_id:= Trim(DM.gate_id);
   ...
   try
     ...
     qTimeShift.Close;
     qTimeShift.Params.ParamByName('pshift').Value:= edtShift.Text;
     qTimeShift.Open;

     qJmlPelanggan.Close;
     qJmlPelanggan.Params.ParamByName('ptgl').Value:= dt;
     qJmlPelanggan.Params.ParamByName('pshift').Value:= Sft;
     qJmlPelanggan.Params.ParamByName('pgate_id').Value:= gt_id;
     qJmlPelanggan.Open;
...
//     MemMasterListlogo2.LoadFromFile('Images/logo2.jpg');
     ...
     MemMasterListkota.AsString:= DM.Perusahaankota.AsString;
     MemMasterListtgl.AsString:= FormatDateTime('dddd, dd-mm-yyyy',dtpStart.Date);
     MemMasterListvnow.AsString:= FormatDateTime('dd-mm-yyyy hh:mm:ss',now);
     MemMasterListdt_from_shift.AsString:= FormatDateTime('hh:mm:ss', qTimeShifttawal.AsDateTime);
     MemMasterListdt_end_shift.AsString:= FormatDateTime('hh:mm:ss', qTimeShifttakhir.AsDateTime);
...
//
//    if pgcMaster.ActivePageIndex=0 then begin
//        ttl:='Rep_Parkir_Shift_'+edtShift.Text+'_Tgl_'+dt;
//
...
//    else
//    if pgcMaster.ActivePageIndex=1 then begin
//        ttl:='Lap_Parkir_Shift_'+edtShift.Text+'_Tgl_'+dt;
//
